 ...
 */
    public function twitter_fetch_items($keyword, $camp)
    {
        ...
        $wp_automatic_tw_bearer_token = trim(get_option('wp_automatic_tw_bearer_token', ''));
        ...
        if (stristr($keyword, 'from:') && !stristr(trim($keyword), ' ')) {
            ...
            if (trim($user_id) == '' || !is_numeric($user_id)) {

                echo '<br>Getting numeric user ID from twitter by username = ' . $userKey;

                $url = 'https://api.twitter.com/2/users/by/username/' . urlencode(trim($userKey));
        ...
        if (stristr($query, 'filter:')) {
            ...
            $query = str_replace('filter:', 'has:', $query);
        ...
        foreach ($keywords as $keyword) {
            ...
            if (trim($keyword) != '') {
                ...
                if (count($res) > 0) {
                    ...
                    $temp['item_author_profile_image'] = str_replace('normal', '200x200', $item->includes->users[0]->profile_image_url);
                    ...
                    if(  in_array('OPT_TW_EXPAND', $camp_opt) ){
                        if(isset($item->data->entities->urls)){
                                foreach ($item->data->entities->urls as $single_url){               
                                    $temp['item_description'] = str_replace( 'href="' . $single_url->url . '"' , 'href="'. $single_url->expanded_url . '"' , $temp['item_description'] );
                                    $temp['item_description'] = str_replace( '>' . $single_url->url . '<' , '>' .  $single_url->display_url . '<' , $temp['item_description'] );
                                }
                        }
                    }
