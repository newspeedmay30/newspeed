	...
	class WpAutomaticYoutube extends wp_automatic {
		...
		function youtube_fetch_links($keyword, $camp) {
			...
			if (in_array ( 'OPT_YT_PLAYLIST', $camp_opt )) {
				...
				//playlist id 
				$playlistID = trim ( $camp_general ['cg_yt_playlist'] );
				...
				if( stristr($playlistID , 'https://www.youtube.com/playlist?list=') ){
					$playlistID = str_replace('https://www.youtube.com/playlist?list=' , '' , $playlistID);
				...
				$search_url = "https://www.googleapis.com/youtube/v3/playlistItems?part={$part}&playlistId=" . $playlistID . "&key=" . trim ( $wp_automatic_yt_tocken ) . "&maxResults=50";
			}
